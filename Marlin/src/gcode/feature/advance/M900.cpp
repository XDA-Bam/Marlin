...
void GcodeSuite::M900() {
  ...
  float &ref_O = planner.extruder_advance_Ko[tool_index], new_O = ref_O;
  const float old_O = new_O;
    ...
    if (parser.seenval('O')) {
      const float O = parser.value_float();
      if (WITHIN(O, -10, 0)) {
        new_O = O;
      }
      else
  ...
  if (new_C != old_C || new_D != old_D || new_O != old_O) {
    planner.synchronize();
    ref_C = new_C;
    ref_D = new_D;
    ref_O = new_O;
  ...
  if (!parser.seen_any()) {
      ...
      SERIAL_ECHO_START();
      #if EXTRUDERS < 2
        SERIAL_ECHOLNPAIR("Advance C=", planner.extruder_advance_K[0], " D=", planner.extruder_advance_Kd[0], " A=", planner.extruder_advance_Ko[0]);
      #else
